...
def call_model_multi_turn(client, instructions, messages):
    """Call OpenAI API with conversation history using chat completions"""
...
def run_conversation(client, instructions, mode_name):
    ...
    t0 = time.time()
    response_1 = call_model_multi_turn(client, instructions, conversation)
    t1 = time.time() - t0
    ...
    t0 = time.time()
    response_2 = call_model_multi_turn(client, instructions, conversation)
    t2 = time.time() - t0
    ...
    t0 = time.time()
    response_3 = call_model_multi_turn(client, instructions, conversation)
    t3 = time.time() - t0
...
def main():
    ...
    print("=" * 70)
    print("CLAIRE'S PROMPT TEST — MULTI-TURN")
    print("=" * 70)
    print(f"Model: {MODEL}")
    ...
    # Save results
    results = {
        "test": "Claire's Prompt - Multi-turn",
        "model": MODEL,
        "temperature": TEMPERATURE,
    ...
    with open("claire_test_results.json", "w") as f:
        json.dump(results, f, indent=2, ensure_ascii=False)
    
    print("\n" + "=" * 70)
    print("Results saved to claire_test_results.json")
    print("=" * 70)
